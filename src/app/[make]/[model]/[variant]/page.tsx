...
import { SPEC_SECTIONS, FUEL_TYPE_LABELS, GEARBOX_LABELS, DRIVETRAIN_LABELS } from '@/lib/constants'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://cardata.wiki/${params.make}/${params.model}/${variant.slug}` },
  }
}
...
export default async function VariantPage({ params }: Props) {
  ...
  if (!variant || variant.model.slug !== params.model || variant.model.make.slug !== params.make) notFound()

  const m = variant.model
  const make = m.make

  // Build schema.org Car
  const schema = {
    '@context': 'https://schema.org',
    '@graph': [
      {
        '@type': 'Car',
        '@id': `https://cardata.wiki/${params.make}/${params.model}/${variant.slug}#vehicle`,
        ...
        url: `https://cardata.wiki/${params.make}/${params.model}/${variant.slug}`,
      },
      {
        '@type': 'Dataset',
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: m.name,      item: `https://cardata.wiki/${params.make}/${params.model}` },
          { '@type': 'ListItem', position: 4, name: variant.name, item: `https://cardata.wiki/${params.make}/${params.model}/${variant.slug}` },
        ],
      },
    ],
  }
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
        ...
        <div className="flex flex-wrap gap-2 self-start">
          <Link href={`/${params.make}/${params.model}/${params.variant}/history`} className="btn-ghost text-xs">
            View edit history
          </Link>
          {session && (
      ...
      <div className="mt-10">
        ...
        <Link href={`/${params.make}/${params.model}`} className="btn-outline text-sm">
